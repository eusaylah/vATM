...
\begin{enumeratesc}
    ...
    \itemsc{Longitudinal separation minima based on time}
    \begin{enumeratescit}
        \itemscit{Aircraft maintaining the same level}
        \begin{enumerate}
            \item \textit{Aircraft flying on the same track:}
            \begin{enumalph}
                %References
                \item 15 minutes (see Figure 5-10); or
                \item 10 minutes, if navigation aids permit frequent determination of position and speed (see Figure 5-11); or
                \item \label{5.4.2.2.1.1c} 5 minutes in the following cases, provided that in each case the preceding aircraft is maintaining a true airspeed of 20 kt or more faster than the succeeding aircraft (see Figure 5-12):
                \begin{enumarab}
                    \item between aircraft that have departed from the same aerodrome;
                    \item between en-route aircraft that have reported over the same exact significant point;
                    \item between departing and en-route aircraft after the en-route aircraft has reported over a fix that is so located in relation to the departure point as to ensure that five-minute separation can be established at the point the departing aircraft will join the air route; or
                \end{enumarab}

                \item 3 minutes in the cases listed under \ref{5.4.2.2.1.1c} provided that in each case the preceding aircraft is maintaining a true airspeed of 40 kt or more faster than the succeeding aircraft (see Figure 5-13).
            \end{enumalph}

            \item \textit{Aircraft flying on crossing tracks:}
            \begin{enumalph}
                %References
                \item 15 minutes at the point of intersection of the tracks (see Figure 5-14); or
                \item 10 minutes if navigation aids permit frequent determination of position and speed (see Figure 5-15).
            \end{enumalph}
        \end{enumerate}

        %Figures 5-10 through 5-15

        \itemscit{Aircraft climbing or descending}
        \begin{enumerate}
            \item \textit{Aircraft on the same track.} When an aircraft will pass through the level of another aircraft on the same track, the following minimum longitudinal separation shall be provided:
            %References
            \begin{enumalph}
                \item 15 minutes while vertical separation does not exist (see Figures 5-16A and 5-16B); or
                \item 10 minutes while vertical separation does not exist, provided that such separation is authorized only where ground-based navigation aids or GNSS permit frequent determination of position and speed (see Figures 5-17A and 5-17B); or
                \item 5 minutes while vertical separation does not exist, provided that:
                \begin{enumarab}
                    \item the level change is commenced within 10 minutes of the time the second aircraft has reported over a common point which must be derived from ground-based navigation aids or by GNSS; and
                    \item when issuing the clearance through third party communication or CPDLC a restriction shall be added to the clearance to ensure that the 10-minute condition is satisfied (see Figures 5-18A and 5-18B).
                \end{enumarab}
            \end{enumalph}

            \note{To facilitate application of the procedure where a considerable change of level is involved, a descending aircraft may be cleared to some convenient level above the lower aircraft, or a climbing aircraft to some convenient level below the higher aircraft, to permit a further check on the separation that will be obtained while vertical separation does not exist.}

            \item \textit{Aircraft on crossing tracks:}%References
            \begin{enumalph}
                \item 15 minutes while vertical separation does not exist (see Figures 5-19A and 5-19B); or
                \item 10 minutes while vertical separation does not exist if navigation aids permit frequent determination of position and speed (see Figures 5-20A and 5-20B).
            \end{enumalph}

            %References
            \item \textit{Aircraft on reciprocal tracks.} Where lateral separation is not provided, vertical separation shall be provided for at least ten minutes prior to and after the time the aircraft are estimated to pass, or are estimated to have passed (see Figure 5-21). Provided it has been determined that the aircraft have passed each other, this minimum need not apply.
        \end{enumerate}

        %Figures 5-16 through 5-21
    \end{enumeratescit}

    \itemsc{Longitudinal separation minima based on distance using \\ distance measuring equipment (DME) and/or GNSS}
    \begin{enumerate}
        \note{Where the term “on track” is used in the provisions relating to the application of longitudinal separation minima using DME and/or GNSS, it means that the aircraft is flying either directly inbound to or directly outbound from the station/waypoint.}
        ...
        \note{For the purpose of applying GNSS-based separation minimum, a distance derived from an integrated navigation system incorporating GNSS input is regarded as equivalent to GNSS distance.}
        \item When applying these separation minima between any aircraft with area navigation capability, controllers shall specifically request GNSS-derived distance.
        \note{Reasons making a pilot unable to provide GNSS distance information may include inadequate on-board equipment, no GNSS input into an integrated navigation system, or a loss of GNSS integrity.}

        \itemscit{Aircraft at the same cruising level}
        \begin{enumerate}
            \item \textit{Aircraft on the same track:}
            \begin{enumalph}
                \item 20 NM, provided:
                \begin{enumarab}
                    \item each aircraft utilizes:
                    \begin{enumroman}
                        \item the same “on-track” DME station when both aircraft are utilizing DME; or
                        \item an “on-track” DME station and a collocated waypoint when one aircraft is utilizing DME and the other is utilizing GNSS; or
                        \item the same waypoint when both aircraft are utilizing GNSS; and
                    \end{enumroman}
                    %References
                    \item separation is checked by obtaining simultaneous DME and/or GNSS readings from the aircraft at frequent intervals to ensure that the minimum will not be infringed (see Figure 5-22);
                \end{enumarab}

                \item 10 NM, provided:
                \begin{enumarab}
                    \item the leading aircraft maintains a true airspeed of 20 kt or more faster than the succeeding aircraft;
                    \item each aircraft utilizes:
                    \begin{enumroman}
                        \item the same “on-track” DME station when both aircraft are utilizing DME; or
                        \item an “on-track” DME station and a collocated waypoint when one aircraft is utilizing DME and the other is utilizing GNSS; or
                    ...
                    \end{enumroman}
                    %References
                    \item separation is checked by obtaining simultaneous DME and/or GNSS readings from the aircraft at such intervals as are necessary to ensure that the minimum is established and will not be infringed (see Figure 5-23).
                \end{enumarab}
            \end{enumalph}

            %References
            \item \textit{Aircraft on crossing tracks.} The longitudinal separation prescribed in 5.4.2.3.3.1 shall also apply provided each aircraft reports distance from the DME station and/or collocated waypoint or same waypoint located at the crossing point of the tracks and that the relative angle between the tracks is less than 90 degrees (see Figures 5-24A and 5-24B).
        \end{enumerate}

        %Figures 5-22 through 5-24

        \itemscit{Aircraft climbing and descending}
        \begin{enumerate}
            \item Aircraft on the same track: 10 NM while vertical separation does not exist, provided:
            \begin{enumalph}
                \item each aircraft utilizes:
                \begin{enumroman}
                    \item the same “on-track” DME station when both aircraft are utilizing DME; or
                    ...
                    \item the same waypoint when both aircraft are utilizing GNSS; and
                \end{enumroman}
                \item one aircraft maintains a level while vertical separation does not exist; and
                %References
                \item separation is established by obtaining simultaneous DME and/or GNSS readings from the aircraft (see Figures 5-25A and 5-25B).
            \end{enumalph}

            \note{To facilitate application of the procedure where a considerable change of level is involved, a descending aircraft may be cleared to some convenient level above the lower aircraft, or a climbing aircraft to some convenient level below the higher aircraft, to permit a further check on the separation that will be obtained while vertical separation does not exist.}

            \item \textit{Aircraft on reciprocal tracks.} Aircraft utilizing on-track DME and/or collocated waypoint or same waypoint may be cleared to climb or descend through the levels occupied by other aircraft utilizing on-track DME and/or collocated waypoint or same waypoint, provided that it has been positively established that the aircraft have passed each other and are at least 10 NM apart, or such other value as prescribed by the appropriate ATS authority.
        \end{enumerate}

        %Figure 5-25
    \end{enumerate}

    \itemsc{Longitudinal separation minima with \\ Mach number technique based on time}
    \begin{enumerate}
        ...
        \begin{enumalph}
            \item the aircraft concerned have reported over the same common point and follow the same track or continuously diverging tracks until some other form of separation is provided; or
            \item if the aircraft have not reported over the same common point and it is possible to ensure, by radar, ADS-B or other means, that the appropriate time interval will exist at the common point from which they either follow the same track or continuously diverging tracks;
        \end{enumalph}
        ...
        \begin{enumarab}[labelindent=2\parindent]
            \item 10 minutes; or
            \item between 9 and 5 minutes inclusive, provided that:
            \par\noindent the preceding aircraft is maintaining a true Mach number greater than the following aircraft in accordance with the following:
            \begin{enumerate}[label=---,labelsep=0.3cm,leftmargin=*,labelindent=0pt]
                \item 9 minutes, if the preceding aircraft is Mach 0.02 faster than the following aircraft;
                \item 8 minutes, if the preceding aircraft is Mach 0.03 faster than the following aircraft;
                \item 7 minutes, if the preceding aircraft is Mach 0.04 faster than the following aircraft;
                \item 6 minutes, if the preceding aircraft is Mach 0.05 faster than the following aircraft;
                \item 5 minutes, if the preceding aircraft is Mach 0.06 faster than the following aircraft;
            \end{enumerate}
        \end{enumarab}

        \item When the 10-minute longitudinal separation minimum with Mach number technique is applied, the preceding aircraft shall maintain a true Mach number equal to or greater than that maintained by the following aircraft.
    \end{enumerate}

    \itemsc{Longitudinal separation minima with Mach number \\ technique based on distance using RNAV}
    \begin{enumerate}
        \note{Guidance material on RNAV operations is contained in the \textup{Performance-based Navigation (PBN) Manual} (Doc 9613).}
        \item Turbojet aircraft shall adhere to the true Mach number approved by ATC and shall request ATC approval before making any changes thereto. If it is essential to make an immediate temporary change in the Mach number (e.g. due to turbulence), ATC shall be notified as soon as possible that such a change has been made.
        ...
        %References
        \begin{enumalph}
            \item each aircraft reports its distance to or from the same “on-track” common point;
            \item separation between aircraft at the same level is checked by obtaining simultaneous RNAV distance readings from the aircraft at frequent intervals to ensure that the minimum will not be infringed (see Figure 5-26);
            \item separation between aircraft climbing or descending is established by obtaining simultaneous RNAV distance readings from the aircraft (see Figures 5-27A and 5-27B); and
            \item in the case of aircraft climbing or descending, one aircraft maintains a level while vertical separation does not exist.
        \end{enumalph}
    ...
    %Figures 5-25 through 5-27

    \itemsc{Longitudinal separation minima based on distance \\ using RNAV where RNP is specified} \label{5.4.2.6}
    \begin{enumerate}
        \note{Guidance material is contained in Attachment B to Annex 11, the \textup{Performance-based Navigation (PBN) Manual} (Doc 9613), the \textup{Air Traffic Services Planning Manual} (Doc 9426) and the \textup{Manual on Airspace Planning Methodology for the Determination of Separation Minima} (Doc 9689).}
        ...
        \note{The term “on track” means that the aircraft is flying either directly inbound to or directly outbound from the station or waypoint.}
        ...
        \begin{enumerate}
            ...
            \note{The communication criteria which are used as a basis for the derivation of the separation minima in this section are set out in Appendix 5 of the \textup{Manual on Airspace Planning Methodology for the Determination of Separation Minima} (Doc 9689). Guidance material for CPDLC is contained in the \textup{Manual of Air Traffic Services Data Link Applications} (Doc 9694).}
            \begin{enumerate}
        ...
        \itemscit{Longitudinal distance-based separation minima \\ in an RPN RNAV environment not using ADS-C}
        \begin{enumerate}
            \item For aircraft cruising, climbing or descending on the same track, the following separation minimum may be used:
            
            \begin{tablecenter}[12pt]{ScScScScSc}
                \toprule
                \makecell{\itshape Separation \\ minimum} & \makecell{\itshape RNP \\ type} & \makecell{\itshape Communication \\ requirement} & \makecell{\itshape Surveillance \\ requirement} & \makecell{\itshape Distance verification \\ requirements} \\
                \midrule
                50 NM & 10 & \makecell{Direct controller-pilot \\ communications} & \makecell{Procedural \\ position reports} & \makecell{At least every \\ 24 minutes} \\
                \bottomrule
            \end{tablecenter}

            \note[1]{Where a considerable change of level is involved using distance-based separation, a descending aircraft may be cleared to some convenient level above the lower aircraft, or a climbing aircraft to some convenient level below the higher aircraft (e.g. 4 000 ft or less) to permit a further check on the separation that will be maintained while vertical separation does not exist.}
            \note[2]{It should be noted that the separation minimum depicted above is based on safety assessments performed specifically for a particular network of tracks or routes. As such, the assessments evaluated traffic characteristics which might be unique to the network being assessed.}
            \note[3]{The separation minimum above was developed in accordance with a collision risk analysis which dictates conditions under which this separation can be applied.}
            \note[4]{Detailed information on the analysis used to determine the separation minimum and on performing safety assessments is contained in the \textup{Manual on Airspace Planning Methodology for the Determination of Separation Minima} (Doc 9689).}
